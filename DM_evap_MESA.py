...
def R311_3(r, Tx, mx, sigma, star):
    '''caleb's version of goulde 3.11, using for polytrope calc'''
    ...
    mu_p = mu_plus(mu(mx))
    mu_m = mu_minus(mu(mx))
    nuu = nu_poly(r, mx, Tx, star)
    xi_g = xi_2_poly(r, mx, Tx, star)
    Ee = E_e(mx, v_esc_poly(r, star))
    Ec = E_c(mx, v_c_poly(r, star))
    g_p = gamma_2_poly('+', r, mx, Tx, v_esc_poly(r, star), v_c_poly(r, star), star)
    g_m = gamma_2_poly('-', r, mx, Tx, v_esc_poly(r, star), v_c_poly(r, star), star)
    a_p = alpha_poly('+', r, mx, v_esc_poly(r, star), v_c_poly(r, star), star)
    a_m = alpha_poly('-', r, mx, v_esc_poly(r, star), v_c_poly(r, star), star)
    b_p = beta_poly('+', r, mx, v_esc_poly(r, star), v_c_poly(r, star), star)
    b_m = beta_poly('-', r, mx, v_esc_poly(r, star), v_c_poly(r, star), star)
    R_pre = 2/np.pi * proton_speed(xi, star) * star.get_vesc_surf() * 1/np.sqrt(mu(mx)) * (T/Tx)**(3/2) * sigma * nc * eta_poly(r, star)
    term1 = np.exp(-(mu_p/xi_g)**2 * (Ee/(kb*Tx))) * (mu(mx)*mu_m/(nuu*xi_g)*(xi_g**2/nuu - mu_p*mu_m/mu(mx)) + mu_p**3/(xi_g*(nuu-mu(mx))))*chi(g_m, g_p)
    term2 = np.exp(-Ec/(kb*Tx)) * mu(mx)/nuu * ( a_p*a_m - 1/(2*mu(mx)) + mu_m**2*(1/mu(mx) - 1/nuu) ) * chi(a_m, a_p)
    term3 = np.exp(-Ec/(kb*Tx)) * np.exp(-(Ee-Ec)/(kb*T)) * mu_p**2/(nuu - mu(mx)) * chi(b_m, b_p)
    term4 = np.exp(-(Ec/(kb*Tx) + a_m**2)) * mu(mx)*a_p/(2*nuu)
    term5 = np.exp(-(Ec/(kb*Tx) + a_p**2)) * mu(mx)*a_m/(2*nuu)
    R_val = R_pre * (term1 + term2 - term3 - term4 + term5)
    return R_val
...
def R311_4(r, Tx, mx, sigma):
    ...
    Ee = E_e(mx, v_esc(r))
    Ec = E_c(mx, v_c(r))
    g_p = gamma_2('+', r, mx, Tx, v_esc(r), v_c(r))
    g_m = gamma_2('-', r, mx, Tx, v_esc(r), v_c(r))
    a_p = alpha('+', r, mx, v_esc(r), v_c(r))
    a_m = alpha('-', r, mx, v_esc(r), v_c(r))
    b_p = beta('+', r, mx, v_esc(r), v_c(r))
    b_m = beta('-', r, mx, v_esc(r), v_c(r))
    a1 = (2/np.pi) * np.sqrt((2*k_cgs*T(r))/(mx))
    a2 = (T(r)/Tx)**(1.5) * sigma * n_p(r) * n_chi(r, Tx, mx)
    R_pre = a1*a2
    # R_pre = 2/np.pi * proton_speed(xi, star) * star.get_vesc_surf() * 1/np.sqrt(mu(mx)) * (T/Tx)**(3/2) * sigma * nc * eta_poly(r, star)
    term1 = np.exp(-(mu_p/xi_g)**2 * (Ee/(kb*Tx))) * (mu(mx)*mu_m/(nuu*xi_g)*(xi_g**2/nuu - mu_p*mu_m/mu(mx)) + mu_p**3/(xi_g*(nuu-mu(mx))))*chi(g_m, g_p)
    term2 = np.exp(-Ec/(kb*Tx)) * mu(mx)/nuu * ( a_p*a_m - 1/(2*mu(mx)) + mu_m**2*(1/mu(mx) - 1/nuu) ) * chi(a_m, a_p)
    term3 = np.exp(-Ec/(kb*Tx)) * np.exp(-(Ee-Ec)/(kb*TT)) * mu_p**2/(nuu - mu(mx)) * chi(b_m, b_p)
    term4 = np.exp(-(Ec/(kb*Tx) + a_m**2)) * mu(mx)*a_p/(2*nuu)
    term5 = np.exp(-(Ec/(kb*Tx) + a_p**2)) * mu(mx)*a_m/(2*nuu)
    R_val = R_pre * (term1 + term2 - term3 - term4 + term5)
    return R_val
...
def alpha_poly(pm, r, m_chi, w, v, star):
    '''made up goulde function'''
    if (pm == '+'):
        val = (m_p/(2 * k_cgs * T_poly(r, star)))**(1/2) * (mu_plus(mu(m_chi)) * v + mu_minus(mu(m_chi)) * w)
    if (pm == '-'):
        val = (m_p/(2 * k_cgs * T_poly(r, star)))**(1/2) * (mu_plus(mu(m_chi)) * v - mu_minus(mu(m_chi)) * w)
    return val
...
def gamma_2_poly(pm, r, m_chi, T_chi, w, v, star):
    '''made up goulde function'''
    if (pm == '+'):
        val = np.sqrt(m_p / (2 * k_cgs * T_poly(r, star))) * (grho_poly(r, m_chi, T_chi, star)*v + xi_2_poly(r, m_chi, T_chi, star)*w)
    if (pm == '-'):
        val = np.sqrt(m_p / (2 * k_cgs * T_poly(r, star))) * (grho_poly(r, m_chi, T_chi, star)*v - xi_2_poly(r, m_chi, T_chi, star)*w)
    return val
...
def grho_poly(r, m_chi, T_chi, star):
    '''made up goulde function'''
    val =  (mu_plus(mu(m_chi)) * (mu_minus(mu(m_chi))))/ xi_2_poly(r, m_chi, T_chi, star)
    return val
...
def xi_2_poly(r, m_chi, T_chi, star):
    '''not the polytope xi!!!!!!, just a made up goulde function'''
    val =  np.sqrt(mu_minus(mu(m_chi))**2 + nu_poly(r, m_chi, T_chi, star))
    return val
...
def nu_poly(r, m_chi, T_chi, star):
    '''made up goulde function'''
    val =  mu(m_chi) * T_poly(r, star) / T_chi
    return val
...
def beta_poly(pm, r, m_chi, w, v, star):
    '''made up goulde function'''
    if (pm == '+'):
        val = (m_p/(2 * k_cgs * T_poly(r, star)))**(1/2) * (mu_minus(mu(m_chi)) * v + mu_plus(mu(m_chi)) * w)
    if (pm == '-'):
        val = (m_p/(2 * k_cgs * T_poly(r, star)))**(1/2) * (mu_minus(mu(m_chi)) * v - mu_plus(mu(m_chi)) * w)
    return val
...
def evap_rate_integrand(r, T_chi, m_chi, sigma):
    ...
    r311 = r**2 * n_chi(r, T_chi, m_chi) * R311_4(r, T_chi, m_chi, sigma) / normfactor(r, m_chi, T_chi)
    return r311
...
def evap_rate_integrand_poly(r, T_chi, m_chi, sigma, star):
    '''the integrand that evap_rate() will evaluate'''
    r311 = r**2 * n_chi_poly(m_chi, r, star, T_chi) * R311_3(r, T_chi, m_chi, sigma, star) / normfactor_poly(r, m_chi, T_chi, star)
    return r311

def evap_rate_lower_integrand_poly(r, T_chi, m_chi, star):
    '''the lower integrand that evap_rate() will evaluate'''
    n = r **2 * n_chi_poly(m_chi, r, star, T_chi)
    return n

def evap_rate_poly(T_chi, m_chi, sigma, star):
    '''evaporation rate of DM for the whole star'''
    top = quad(evap_rate_integrand_poly, 0, star.get_radius_cm(), args=(T_chi, m_chi, sigma, star))[0]
    bottom = quad(evap_rate_lower_integrand_poly, 0, star.get_radius_cm(), args=(T_chi, m_chi, star), limit=1000)[0]
    # print("top = ", top)
    # print("bottom = ", bottom)
    return top/bottom
...
def read_in_tau(name):
    ...
    with open(name) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
            tau_csv.append(float(row[1]))
            mu_csv.append(float(row[0]))

    # now fit interpolation functions to T_chi w.r.t m_chi
    tau_fit = interp(mu_csv, tau_csv)
    return (mu_csv, tau_csv, tau_fit)
...
def main():
    ...
    parser.add_argument("-P", "--poly", help="calc the evap polytrope, give the CSV file holding the tau vs. mu data for an N=3 polytrope", type=str)
    parser.add_argument("-S", "--star", help="100, 300, or 1000", type=str)
    ...
    # use polytrop that corresponds to the mass of the MESA star
    if args.direc == '1' or args.direc == '4':
        star = M100
    if args.direc == '2' or args.direc == '5':
        star = M300
    if args.direc == '3' or args.direc == '6':
        star = M1000
    ...
    if args.direc and args.profile:
        ...
        if args.TchiMchi:
            ...
            name = "TM" + str(args.direc) +"_" + str(args.profile)
        ...
        if args.MESA:
            ...
            r_poly = np.linspace(prof.radius_cm[-1], star.get_radius_cm(), 100)
            T_poly_sample = []
            rho_poly_sample = []
            phi_poly_sample = []
            n_p_poly_sample = []
            ...
            for i in range(len(r_poly)):
                T_poly_sample.append(T_poly(r_poly[i], star))
                rho_poly_sample.append(rho_poly(r_poly[i], star))
                phi_poly_sample.append(phi_poly(r_poly[i], star))
                n_p_poly_sample.append(n_p_poly(r_poly[i], star))
                n_chi_poly_sample.append(n_chi_poly(m_chi, r_poly[i], star, T_chi))
                v_esc_poly_sample.append(v_esc_poly(r_poly[i], star))
            ...
            plt.title("Density: MESA (Windhorst) vs. N=3")
            plt.legend()
            plt.xlabel("$r$ [cm]")
            ...
            plt.axhline(y=T_chi_fit(1*g_per_GeV), label="$T_{\chi}$ for $1$ GeV", c="#DD2BE2", linewidth=2, ls=':')
            plt.axhline(y=T_chi_fit(0.1*g_per_GeV), label="$T_{\chi}$ for $10^{-1}$ GeV", c="#8A2BE2", linewidth=2, ls=':')
            plt.axhline(y=T_chi_fit(0.01*g_per_GeV), label="$T_{\chi}$ for $10^{-2}$ GeV", c="#eb4242", linewidth=2, ls=':')
            plt.title("Temperature: MESA (Windhorst) vs. N=3")
            plt.legend()
            plt.xlabel('$r$ [cm]')
            plt.ylabel('$T$ [K]')
            ...
            plt.title("Proton Number Density: MESA (Windhorst) vs. N=3")
            plt.legend()
            # plt.yscale("log")
            # plt.xscale("log")
            plt.xlabel("$r$ [cm]")
            ...
            plt.title("DM Number Density: MESA (Windhorst) vs. N=3")
            plt.legend()
            # plt.yscale("log")
            # plt.xscale("log")
            plt.xlabel("$r$ [cm]")
            plt.ylabel("$n_{\chi}$ [$cm^{-3}$]")
            file = "nchi_" + str(args.direc) + "_" + str(args.profile) + "_" + str(int(math.log10(m))) +  ".png"
            plt.savefig(file, dpi=400)
            plt.clf()
            ...
            plt.title("Escape Velocity: MESA (Windhorst) vs. N=3")
            plt.legend()
            plt.xlabel("$r$ [cm]")
            ...
            plt.title("Gravitational Potential: MESA (Windhorst) vs. N=3")
            plt.legend()
            plt.xlabel("$r$ [cm]")
        ...
        if args.R311:
            ...
            print("#######################")
            print("DM Mass =", m, "GeV")
            print("DM Temperature for that mass in this star =", T_chi_fit(m*g_per_GeV), "K")
            print("#######################")
            ...
            for i in range(len(r)):
                ...
                R311_new.append(R311_4(r[i], T_chi_fit(m*g_per_GeV), m*g_per_GeV, sigma))
                R311_caleb.append(R311_3(r[i], T_chi_fit(m*g_per_GeV), m*g_per_GeV, sigma, star))
                norm.append(normfactor(r[i], m*g_per_GeV, T_chi_fit(m*g_per_GeV)))
                norm_poly.append(normfactor_poly(r[i], m*g_per_GeV, T_chi_fit(m*g_per_GeV), star))
                rate.append(R311_sample[i]/norm[i])
                # if (T_chi_fit(m*g_per_GeV) - T(r[i]))/T(r[i]) < 0.0001:
                #     tsame.append(r[i])
            ...
            plt.plot(r, R311_new, ls = '-', linewidth = 2, label="R 3.11 fixed " + mesa_lab)
            plt.plot(r, R311_caleb, ls = ':', linewidth = 2, label="R 3.11 caleb ")
            plt.plot(r, R310_sample, ls = '--', linewidth = 2, label="R 3.10 " + mesa_lab)
            # plt.plot(r, R311, ls = '-', linewidth = 2, label="R 3.11 " + mesa_lab)
            # if tsame:
            #     plt.axvline(x=tsame[-1], label="$T_{\chi} = T(r)$", c="#8A2BE2", linewidth=2)
            plt.title("MESA Gould Eq. 3.10 " + lab_mass  + "$M_{\\odot}$ (Windhorst), sigma = " + str(sigma) + " , $m_{\chi} =$ " + str(m) + "GeV")
            plt.legend()
            plt.xlabel('$r$ [cm]')
            plt.ylabel('$R(w|v)$ [???]')
            # plt.ylim(0, 1.5*10**(-9))
            plt.yscale("log")
            # plt.xscale("log")
            file = "R311_" + str(args.direc) + "_" + str(args.profile) + "_" + str(int(math.log10(m))) + ".png"
            plt.savefig(file, dpi=400)
            # plt.show()
            plt.clf()
            ...
            plt.title("MESA Gould Normalization Factor " + lab_mass  + "$M_{\\odot}$ (Windhorst), sigma = " + str(sigma) + " , $m_{\chi} =$ " + str(m) + "GeV")
            plt.legend()
            plt.xlabel('$r$ [cm]')
            plt.ylabel('')
            plt.yscale("log")
            # plt.xscale("log")
            file = "norm_" + str(args.direc) + "_" + str(args.profile) + "_" + str(int(math.log10(m))) + ".png"
            plt.savefig(file, dpi=400)
            # plt.show()
            plt.clf()
        ...
        if args.Evap:
            ...
            for i in range(len(m_chi_csv)):
                ...
                caleb.append(evap_rate_poly(T_chi_csv[i], m_chi_csv[i], sigma, star))
                print("Evap. rate is =", evap_sample[i])
                print("Caleb's evap. rate is =", caleb[i])
    ...
    if args.poly:
        '''NOW CALC POLY EVAP RATES'''
        # note: sigma is set at the begining of the main loop
        # intalize arrays
        caleb = []
        m_chi_csv = []
        T_chi_csv = []

        # set the correct star from the command line arguments
        if args.star == "100":
            star = M100
        elif args.star == "300":
            star = M300
            ...
            print("please specific a star of 100, 300, or 1000 solar masses using the \"-p\" flag. tnk u")
            exit()

        # read in previously calculated mu vs. tau data from files
        file = args.poly
        if path.exists(file) == True:
            (mu_csv, tau_csv, tau_fit) = read_in_tau(file)

        # now we need to loop thru mu and tau to convert mu -> m_chi and tau -> T_chi
        # mu is just defined as m_chi / m_proton
        # tau is just defined as T_chi / T(0) where T(r) is the temp of baryonic 
        # matter in the star, i.e. T(0) = the center temp. of the star
        for i in range(len(mu_csv)):
            m_chi_csv.append(mu_csv[i] * m_p)
        for i in range(len(tau_csv)):
            T_chi_csv.append(tau_csv[i] * T_poly(0.0, star))

        # run thru a do loop for all the masses to actually calc the evap
        for i in range(len(m_chi_csv)):
            print("####################################################")
            print("Getting evap. rate for m_chi =", m_chi_csv[i]/g_per_GeV, "GeV...")
            # THIS IS WHERE EVAP IS CALLED!!!
            caleb.append(evap_rate_poly(T_chi_csv[i], m_chi_csv[i], sigma, star))
            print("Caleb's evap. rate is =", caleb[i])

        # lets convert our m_chi (in g) to m_chi (in GeV) for ploting
        m_chi_csv_GeV = []
        for i in range(len(m_chi_csv)):
            m_chi_csv_GeV.append(m_chi_csv[i]/g_per_GeV)
